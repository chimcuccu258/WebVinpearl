...
<div class="w-2/4 mx-auto pt-2">
    <div class="rounded-sm border border-stroke bg-white shadow-default p-4">
        ...
        <form action="{{ route('nhan_viens.store') }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="p-6.5">
                <div class="mb-4.5 flex flex-col gap-6 xl:flex-row">
                    <div class="w-full xl:w-1/2">
                        ...
                        <input name="hoTenNV" value="{{ old('hoTenNV') }}" type="text" placeholder="Nhập họ và tên của bạn" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter">
                        @if($errors->has('hoTenNV'))
                        <span class="text-red-500">{{ $errors->first('hoTenNV') }}</span>
                        @endif
                    </div>
                    ...
                    <div class="w-full xl:w-1/2">
                        ...
                        <div class="relative z-20 bg-transparent">
                            <select name="gioiTinh" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary">
                                ...
                                <option value="Nữ" @if(old('gioiTinh')=='Nữ' ) selected @endif>Nữ</option>
                                <option value="Không" @if(old('gioiTinh')=='Không' ) selected @endif>Không</option>
                            </select>
                ...
                <div class="mb-4.5">
                    ...
                    <textarea name="diaChi" rows="1" placeholder="Nhập địa chỉ của bạn" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter">{{ old('diaChi') }}</textarea>
                ...
                <div class="mb-6">
                    ...
                    <div class="relative z-20 bg-transparent">
                        <select name="maLoaiNV" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary">
                            @foreach($loai_nhan_viens as $lnv)
                            <option value="{{ $lnv->maLoaiNV }}" @if(old('maLoaiNV')==$lnv->maLoaiNV) selected @endif>{{ $lnv->tenLoai }}</option>
                            @endforeach
                        </select>
                        <span class="absolute top-1/2 right-4 z-30 -translate-y-1/2">
                            <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    @if($errors->has('maLoaiNV'))
                    <span class="text-red-500">{{ $errors->first('maLoaiNV') }}</span>
                    @endif
                </div>
